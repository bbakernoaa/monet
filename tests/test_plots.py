...
def bias_scatter_data_xr() -> xr.Dataset:
    ...
    # This simulates a dataset that might come from pairing model and obs data
    # The 'site' dimension would be the common pairing index.
    lats = [34.0, 35.0, 36.0]
    lons = [-118.0, -119.0, -120.0]
    sites = ["site1", "site2", "site3"]
    model_data = [10.0, 12.0, 15.0]
    obs_data = [8.0, 11.0, 16.0]

    ds = xr.Dataset(
        {
            "model": (("site",), model_data),
            "obs": (("site",), obs_data),
            "latitude": (("site",), lats),
            "longitude": (("site",), lons),
        },
...
def test_spatial_bias_scatter_xr_no_ax(bias_scatter_data_xr: xr.Dataset) -> None:
    """Test spatial_bias_scatter with xarray input and no ax."""
    ds = bias_scatter_data_xr
    fig, ax = plots.spatial_bias_scatter(ds)

    assert isinstance(fig, matplotlib.figure.Figure)
    assert isinstance(ax, matplotlib.axes.Axes)
    # Check that a scatter plot was created
    assert len(ax.collections) > 0
    plt.close(fig)


def test_spatial_bias_scatter_xr_with_ax(bias_scatter_data_xr: xr.Dataset) -> None:
    """Test spatial_bias_scatter with xarray input and a provided ax."""
    ds = bias_scatter_data_xr
    fig_in = plt.figure()
    ax_in = fig_in.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
