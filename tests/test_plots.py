...
def test_spatial_bias_scatter_xarray_no_ax(bias_scatter_data_xarray: xr.Dataset) -> None:
    """Test the xarray-based spatial_bias_scatter when no ax is provided."""
    ds = bias_scatter_data_xarray
    fig, ax = plots.spatial_bias_scatter(ds)

    assert isinstance(fig, matplotlib.figure.Figure)
    assert isinstance(ax, matplotlib.axes.Axes)
    assert len(ax.collections) > 0  # Check that a scatter plot was created
    plt.close(fig)


def test_spatial_bias_scatter_xarray_with_ax(bias_scatter_data_xarray: xr.Dataset) -> None:
    """Test the xarray-based spatial_bias_scatter when an ax is provided."""
    ds = bias_scatter_data_xarray
    fig_in = plt.figure()
    ax_in = fig_in.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
